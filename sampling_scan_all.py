...
#To be put in the miscellaneous functions but have not because it is not up to date anyway
...
def scan(i2csocket, daqsocket, scan_injection, process, subprocess, trim_val, gain, calib, active_menu, num_events, calibreq, startBX, stopBX, stepBX, startPhase, stopPhase, stepPhase, injectedChannels, odir):
    ...
    (calib_preamp, calib_conv, IntCtest, choice_cinj, cmd_120p, L_g2, L_g1, L_g0, H_g2, H_g1, H_g0) = configure_injection_assign(scan_injection)
    ...
    for BX in range(startBX, stopBX, stepBX):
        ...
        for phase in range(startPhase,stopPhase+1,stepPhase):
            ...
            if process == 'ext':
              ...
              fout.write("####  BX, PHASE ####" + '\n')
              fout.write("sample_scan: {} bx: {} phase: {} \n".format(index,BX,phase))
...
def Sampling_scan(i2csocket,daqsocket, clisocket, process = 'int', subprocess = 'preamp', basedir,device_name, device_type, injectionConfig,suffix="", active_menu = 'calibAndL1A', num_events = 500, calibreq = 0x10, bxoffset = 21, noofoffsets = 5, stepBX = 1, startPhase=0, stopPhase=15, stepPhase=1):
    #testName='PreampSampling_scan'
    testName = subprocess.capitalize() + 'Sampling_scan' #To keep the test name somewhat consistent with before
    ...
    elif process == 'int' & subprocess == 'preamp':
    ...
    elif process == 'int' & subprocess == 'conv':
    ...
    #pre_injection
    (calib_preamp, calib_conv, IntCtest, choice_cinj, cmd_120p, L_g2, L_g1, L_g0, H_g2, H_g1, H_g0) = configure_injection_assign(pre_injection)
    i2csocket.configure_injection(trim_val = trim_val, process = process, calib_preamp = calib_preamp, calib_conv = calib_conv, gain = gain,injectedChannels = injectedChannels, IntCtest = IntCtest, choice_cinj = choice_cinj, cmd_120p = cmd_120p, L_g2 = L_g2, H_g2 = H_g2, L_g1 = L_g1, H_g1 = H_g1, L_g0 = L_g0, H_g0 = H_g0)    
    ...
    if process == 'int':
      scan(i2csocket=i2csocket, daqsocket=daqsocket, scan_injection, process = process, subprocess = subprocess, trim_val = 0, gain = gain, calib = calib, active_menu = active_menu, num_events = num_events, calibreq = calibreq, startBX=startBX, stopBX=stopBX, stepBX=stepBX, startPhase=startPhase, stopPhase=stopPhase, stepPhase=stepPhase, injectedChannels=injectedChannels, odir=odir)
    elif process == 'ext':
      for trim_val in range(trimstart, trimstop, trimstep):
        scan(i2csocket=i2csocket, daqsocket=daqsocket, scan_injection, process = process, subprocess = subprocess, trim_val = trim_val, gain = gain, calib = calib, active_menu = active_menu, num_events = num_events, calibreq = calibreq, startBX=startBX, stopBX=stopBX, stepBX=stepBX, startPhase=startPhase, stopPhase=stopPhase, stepPhase=stepPhase, injectedChannels=injectedChannels, odir=odir)
  ...
  clisocket.stop()
    ...
    #post_injection
    (calib_preamp, calib_conv, IntCtest, choice_cinj, cmd_120p, L_g2, L_g1, L_g0, H_g2, H_g1, H_g0) = configure_injection_assign(post_injection)
    i2csocket.configure_injection(trim_val = trim_val, process = process, calib_preamp = calib_preamp, calib_conv = calib_conv, gain = gain,injectedChannels = injectedChannels, IntCtest = IntCtest, choice_cinj = choice_cinj, cmd_120p = cmd_120p, L_g2 = L_g2, H_g2 = H_g2, L_g1 = L_g1, H_g1 = H_g1, L_g0 = L_g0, H_g0 = H_g0)    
