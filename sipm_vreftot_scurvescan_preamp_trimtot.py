...
def tot_scurvescan(i2csocket,daqsocket,clisocket,basedir,device_name,device_type,tot_vals,configFile,suffix=''):
    testName = "vreftot_scurvescan"
    odir = misc_func.mkdir(os.path.realpath(basedir),device_type,testName = testName,suffix=suffix)
    with open(configFile) as f:
        config = yaml.safe_load(f)

    for trimtot in tot_vals:
        nestedConf = nested_dict()    
        trimtot_odir = odir+"trim_tot_%s/"%(trimtot)
        print("Directories",trimtot_odir)
        
        for rocId in config.keys():
            if rocId.find('roc_s')==0:
                for channel in range(0,72):
                    nestedConf[rocId]['sc']['ch'][channel]['trim_tot'] = trimtot
                    
        i2csocket.configure(yamlNode=nestedConf.to_dict())
        print("Injection scan for trim_tot",trimtot)
        sipm_injection_scan(i2csocket,daqsocket,clisocket,trimtot_odir,options.dut,options.device_type,suffix=options.suffix,keepRawData=0,analysis=1)

def sipm_injection_scan(i2csocket,daqsocket,clisocket,trimtot_odir,device_name,device_type,suffix='',keepRawData=1,analysis=1):
    ...
    if injection_scan_mode == 1:
        ...
        for batch_num in range(int(len(injectedChannels)/len_batch)):
            ...
            'calib' : [i for i in range(500,2500,20)], #Useful for global tot (tot vref) of 300
            ...
            inj_preamp.sipm_injection_scan(i2csocket,daqsocket,clisocket,trimtot_odir,options.dut,device_type, injectionConfig,scurve_scan = 1,suffix="gain0_ch%i"%batch_num,active_menu = 'calibAndL1AplusTPG',keepRawData=1,analysis=1)
...
if __name__ == "__main__":
    ...
    ############
    tot_vals = [0,4,8,12,16,20,24,28,32] #Same as what I had used last time (for conveyor injection)
    #tot_vals = [32]
    #tot_vals = [0,8,16]
